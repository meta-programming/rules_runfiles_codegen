...
To avoid side-effects in package `init()` blocks and improve testability, the Go library uses a **lazy, explicit** resolution model. The generated library exposes unresolved `FileSpec` and `ExecutableSpec` symbols. The developer must explicitly resolve them at runtime (e.g., during startup in `main()` or lazily when needed) using `.Resolve()` or `.MustResolve()`. The resolved objects (`File` and `Executable`) provide safe, type-safe accessors.
...
func main() {
	...
	// We resolve the spec to a File, handling the error.
	configFile, err := my_resources.ConfigJSON.Resolve()
	if err != nil {
	...
	// Case 2: Data file from a different repository
	// We can also use MustResolve() if we want to panic on failure (fail-fast).
	schemaFile := my_resources.ExternalSchema.MustResolve()
	schemaPath := schemaFile.Path()
	fmt.Printf("Loading external schema from: %s\n", schemaPath)

	// Case 3: Executable target (not self-contained)
	// We resolve to an Executable, then .Cmd() automatically propagates the runfiles env!
	helper := my_resources.HelperTool.MustResolve()
	cmd := helper.Cmd()
...
The generated code is extremely lightweight because all the resolution logic and rich wrappers are deferred to the core `runfile` runtime package:
